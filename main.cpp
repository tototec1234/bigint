...
// ULONG_MAX = 18446744073709551615 (20桁) を超える演算はstoullで壊れる
void	test_overflow_cases(void)
{
	std::cout << "=== Large number tests (would overflow with old stoull) ===" << std::endl;

	// operator+: ULONG_MAX + 1 → 18446744073709551616 (stoullでオーバーフロー)
	bigint	a(18446744073709551615UL);
	bigint	b(1UL);
	std::cout << "ULONG_MAX         = " << a << std::endl;
	std::cout << "ULONG_MAX + 1     = " << (a + b) << std::endl; // 期待値: 18446744073709551616

	// operator+: 巨大数同士の加算
	std::cout << "ULONG_MAX * 2     = " << (a + a) << std::endl; // 期待値: 36893488147419103230
	...
	std::cout << "(ULONG_MAX*2) >> 5 = " << (d >> 5) << std::endl; // 期待値: 368934881474191
}
